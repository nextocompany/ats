...
// The unified "Candidates" surface (Phase 1 of the Candidates+Members unify):
// one account-keyed list of every person — including portal signups with no
// application yet — readable by any role but server-scoped to what the role may
// see. CRM controls (select/bulk/CSV export) stay gated to member admins.
function CandidatesInner() {
  const t = useTranslations("members"); // ported list controls / columns / stats
  const tc = useTranslations("candidates"); // page identity (eyebrow / title / count)
  const locale = useLocale();
  const params = useSearchParams();
  const router = useRouter();
  const { data: me, isLoading: meLoading } = useMe();
  const isAdmin = isMemberAdmin(me); // gates CRM-only controls, not the list itself
  ...
  const { data: stats } = useMemberStats(!meLoading);

  const items = data?.data ?? [];
  const total = data?.meta?.total ?? 0;
  const pages = Math.max(1, Math.ceil(total / LIMIT));
  const allChecked = items.length > 0 && items.every((m) => selected.includes(m.id));
  const cols = isAdmin ? 7 : 6; // checkbox column only renders for admins
  ...
  const exportCsv = async () => {
    ...
    try {
      ...
      await downloadFile(`/api/v1/admin/members/export.csv${qs}`, "candidates.csv");
    } catch (e) {
  ...
  return (
    <div className="settle space-y-6">
      <PageHeader
        eyebrow={tc("eyebrow")}
        title={tc("title")}
        meta={<span className="tabular-nums">{tc("metaCount", { count: total })}</span>}
        actions={
          <>
      ...
      <ul className="space-y-2.5 md:hidden">
        ...
        {items.map((m) => (
          <li key={m.id} className="rounded-xl bg-card ring-1 ring-hairline">
            <Link href={`/candidates/${m.id}`} className="flex items-start gap-3 p-4">
          ...
          <table className="w-full min-w-[820px] text-sm">
            <thead className="ledger-head sticky top-0 z-10 text-left">
              <tr>
                ...
                <th scope="col" className={isAdmin ? "px-3 py-3" : "pl-5 pr-3 py-3"}>{t("colMember")}</th>
            ...
            <tbody>
              {isLoading && Array.from({ length: 8 }).map((_, i) => (
                <tr key={i} className="border-b border-hairline last:border-0">
                  <td className="px-5 py-3.5" colSpan={cols}><Skeleton className="h-5 w-full" /></td>
                </tr>
              ))}
              {!isLoading && items.length === 0 && (
                <tr><td colSpan={cols} className="px-5 py-16 text-center text-muted-foreground">{t("empty")}</td></tr>
              )}
              {items.map((m) => (
                <tr key={m.id} data-sel={selected.includes(m.id)} className="ledger-row group border-b border-hairline last:border-0 data-[sel=true]:bg-brand-soft/30">
                  ...
                  <td className={isAdmin ? "px-3 py-3.5" : "pl-5 pr-3 py-3.5"}>
      ...
      {isAdmin && <MemberBulkBar selected={selected} onDone={() => setSelected([])} />}
    </div>
  );
}
