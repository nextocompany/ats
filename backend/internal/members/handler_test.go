...
// stubRepo is an in-memory Repository double.
...
// role, then mounts the member routes over the stub repo.
func appWithRole(role string, repo Repository) *fiber.App {
	...
	RegisterDashboardRoutes(fa, NewHandler(repo, nil, stubSigner{}))
	return fa
}
