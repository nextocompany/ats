...
**FRONTEND (dashboard only — career-portal unaffected):**
- ✅ **Area management console** — PR #184 (`e1a3a7b`): `/areas` page (gated area.admin), CRUD + store/member assignment, hooks, i18n. tsc+eslint clean; visual UAT pending.
- ✅ read-only HM gating — automatic (permission-driven; HM holds no operate perms).
- ✅ **Lock UI** on candidate detail — PR #184 (`6deec31`): members payload now exposes the canonical `candidate_id`, `/users/me` exposes `local_id`; LockBar (claim / you-are-processing+release / processing-by-X+admin-force). tsc+eslint clean; visual UAT pending.
- ✅ **Role-assignment dropdown** — PR #184 (`4da2b0c`): RoleSelect already data-driven (new roles appear); added curated th/en labels, fixed create-default to hr_store, + AreaCoveragePicker (area_hr) backed by new user-side endpoint `GET/PUT /users/:id/areas`. Backend+integration green; tsc+eslint clean.
- ⛔ **Lock hard-enforcement** (backend): gate mutating candidate handlers to require the lock (currently advisory — claim/display/pickup work, mutations not yet rejected without a lock).
- ⛔ **HM notification** on new candidate in their requisition.
- ⛔ full visual UAT.
