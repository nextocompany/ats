...
**ยังไม่ทำ (P2/P3):** area CRUD admin API+UI, populate vacancy_id at branch-assign, hiring_manager resolve-by-email at req-open, lock acquire/release enforcement + TTL, 3-day release sweep (scheduler+worker), HM notification, read-only HM frontend + approval-chain remap (HM as approver), **old-role cutover + user migration**, wiring Scope.UserID into list/detail call sites.
