...
- ⛔ **Lock UI** on candidate detail — needs the detail payload to expose canonical `candidates.id` first (detail is account-keyed; lock key is canonical candidate id) → small backend change + UI.
